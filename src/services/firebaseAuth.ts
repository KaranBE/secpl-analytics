...
export const googleSignIn = async (): Promise<{ user: User; accessToken: string } | null> => {
  ...
  activeSignInPromise = (async () => {
    try {
      ...
      // Gracefully handle Firebase Auth internal assertion failed / pending promise was never set
      if (
        error?.message?.includes('Pending promise was never set') ||
        error?.message?.includes('INTERNAL ASSERTION FAILED') ||
        error?.toString?.().includes('Pending promise was never set')
      ) {
        console.warn('[Firebase Auth] Handled popup assertion (popup closed or duplicate event).');
        return null;
      }
